<script lang="ts">
	...
	let currentStep = $state<'email' | 'verify' | 'username' | 'complete'>('email');
	let email = $state('');
	...
	onMount(() => {
		const urlPhone = $page.url.searchParams.get('p');
		if (urlPhone) {
	...
	async function handleEmailSubmit(e: Event) {
		...
		try {
			...
			// After magic link is sent, they need to check email
			// The verify page will redirect them back here or to username selection
			currentStep = 'verify';
		} catch (err) {
