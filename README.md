src/
# TBR Delta – SMS-Powered Reading List

Capture books from anywhere (SMS, Amazon links, barcode photos, CSV exports) and review them on a multi-shelf web dashboard. Toggle read/owned status, jot inline notes, and manage your backlog without installing an app.

---

## Highlights

- **SMS-first capture**: START/STOP/HELP/ADD commands, ISBN parsing, Amazon link scraping, barcode OCR via Google Vision, and title/author search fallback.
...
Visit `http://localhost:5173/+15551234567` (replace with your phone number). The UI stores the last user ID in `localStorage` for quick returns.
...
4. **Feedback**: Hit the FAB to file a Trello card; screenshots are optional, failure states are non-blocking.
...
├── lib/
│   ├── components/ui/          # Button, Badge, FlipCard, Feedback modal, etc.
│   └── server/
│       ├── auth.ts             # Referer-based ownership helpers
...
│   ├── +page.svelte            # Landing page with getting started flow
│   ├── about/+page.svelte      # About copy
│   ├── help/+page.svelte       # SMS command reference
│   ├── [username]/+page.*      # Reading list dashboard (grid/list, modal, bulk import)
│   └── api/
│       ├── books/              # add, update, delete, shelves, detect endpoints
